			...
			<div class="row">
				<div class="col-lg-12">
							...
							<div class="col-lg-3 col-md-6 col-sm-6">
								<div class="product__item">
									...
									<div class="product__item__text">
										<h5>
											<a href="review.do?title=${v.title}">${v.title}</a>
										</h5>
									</div>
								</div>
							</div>
					...
					<div class="product__pagination">
						<a href="categories.do?page=1"><i class="fa fa-angle-double-left"></i></a><!-- 처음으로 -->
						<c:if test="${paging.startPage != 1 }">
						<a href="categories.do?page=${(page-1)-(page-1)%paging.perPageSet - paging.perPageSet + 1}"><i class="fa fa-angle-left"></i></a><!-- 이전페이지 -->
						</c:if>
						<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="p">
						<c:if test="${paging.curPage == p}">
						<a href="categories.do?page=${p}" class="current-page">${p}</a>
						</c:if>
						<c:if test="${paging.curPage != p}">
						<a href="categories.do?page=${p}" >${p}</a>
						</c:if>
						</c:forEach>
                        <c:if test="${paging.endPage != paging.lastPage}">
                        <a href="categories.do?page=${(page-1)-(page-1)%paging.perPageSet + paging.perPageSet + 1}"><i class="fa fa-angle-right"></i></a><!-- 다음페이지 -->
                        </c:if>
                        <a href="categories.do?page=${paging.lastPage}"><i class="fa fa-angle-double-right"></i></a><!-- 마지막으로 -->
                    </div>
				</div>
			</div>
		</div>
