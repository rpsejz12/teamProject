            ...
            <div class="anime__details__content">
                    ...
                    <div class="col-lg-9">
                        <div class="anime__details__text">
                            <div class="anime__details__title">
                                <h3>${mdata.title}</h3>
                                <span>${mdata.mtype}  / Date:${mdata.mdate}</span>
                            </div>
                            <p>${mdata.content}</p>
                            </div>
                        </div>
                ...
                <div class="row">
                    <div class="col-lg-12 col-md-12">
                        <div class="anime__details__review">
                            ...
                            <div class="anime__review__item">
                                <div class="anime__review__item__text">
                                	<!-- 주석지우기 -->
                                    <h6>${v.id} - <span>${v.rdate}</span></h6><mytag:delete rpk="${v.rpk}" id="${v.id}" mpk="${v.mpk}"/>
                                    <br>
                                    <p>${v.cmt}</p>
                                </div>
                            </div>
						...
						<a href="r_selectAll.do?page=1&mpk=${mdata.mpk}"><i class="fa fa-angle-double-left"></i></a><!-- 처음으로 -->
						<c:if test="${paging.startPage != 1 }">
						<a href="r_selectAll.do?page=${(page-1)-(page-1)%paging.perPagSet - paging.perPageSet + 1}&mpk=${mdata.mpk}"><i class="fa fa-angle-left"></i></a><!-- 이전페이지 -->
						</c:if>
						<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="p">
						<c:if test="${paging.curPage == p}">
						<a href="r_selectAll.do?page=${p}&mpk=${mdata.mpk}" class="current-page">${p}</a>
						</c:if>
						<c:if test="${paging.curPage != p}">
						<a href="r_selectAll.do?page=${p}&mpk=${mdata.mpk}" >${p}</a>
						</c:if>
						</c:forEach>
                        <c:if test="${paging.endPage != paging.lastPage}">
                        <a href="r_selectAll.do?page=${(page-1)-(page-1)%paging.perPageSet + paging.perPageSet + 1}&mpk=${mdata.mpk}"><i class="fa fa-angle-right"></i></a><!-- 다음페이지 -->
                        </c:if>
                        <a href="r_selectAll.do?page=${paging.lastPage}&mpk=${mdata.mpk}"><i class="fa fa-angle-double-right"></i></a><!-- 마지막으로 -->
                    </div>
                    </div>
                </div>
            </div>
